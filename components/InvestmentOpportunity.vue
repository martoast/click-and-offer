          ...
          <div class="p-4 border border-dashed border-amber-500/30 rounded-lg bg-amber-500/5 flex items-start space-x-3">
             ...
             <div class="flex-grow">
              ...
              <div class="flex gap-3 items-center">
                ...
                <button @click="autoFillMortgageData" class="px-3 py-1.5 rounded-lg border border-blue-700 bg-blue-800/50 hover:bg-blue-700 text-blue-300 text-xs transition">Auto-Fill</button>
          ...
          <div v-if="subtoInputs.existingLoanBalance > 0" class="space-y-6">
                      ...
                      <template v-if="subtoCalculated.sellerCarryAmount > 0">
                          ...
                          <div class="input-group">
                              <label class="input-label">Balloon Payment Term (Years)</label>
                               <div class="relative">
                                  <input v-model.number="subtoInputs.balloonTerm" type="number" min="1" max="30" class="input-field pr-10" placeholder="Balloon term" />
                                  <span class="input-symbol-right pr-3">yrs</span>
                              </div>
                              <p class="input-help">Term for balloon payment (shorter than amortization)</p>
                          </div>
                      </template>
                      <template v-else>
              ...
              <div v-if="subtoInputs.existingLoanPITI > 0" class="pt-6 mt-6 border-t border-slate-700/50">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                       ...
                       <div class="result-box"><div class="result-label">Principal</div><div class="result-value">{{ formatPrice(subtoCalculated.monthlyPrincipal) }}/mo</div></div>
                       <div class="result-box"><div class="result-label">Interest</div><div class="result-value">{{ formatPrice(subtoCalculated.monthlyInterest) }}/mo</div></div>
                       ...
                       <div v-if="subtoInputs.balloonTerm && subtoInputs.balloonTerm < subtoInputs.sellerCarryTerm" class="result-box bg-blue-500/10 border border-blue-500/20">
                          <div class="result-label text-blue-300">Balloon Payment</div>
                          <div class="result-value text-blue-300">{{ formatPrice(subtoCalculated.balloonPayment) }}</div>
                          <div class="result-detail">Due after {{ subtoInputs.balloonTerm }} years</div>
                       </div>
                  </div>
              </div>
...
<div v-if="selectedScenario === 'SellerFinancing'" class="space-y-6">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
      ...
      <div class="space-y-4">
         ...
         <div class="input-group">
            <label class="input-label">Seller Note Amortization (Years)</label>
             <div class="relative">
                <input v-model.number="sellerFinancingInputs.sellerTerm" type="number" min="1" class="input-field pr-10" placeholder="Amortization period" />
                <span class="input-symbol-right pr-3">yrs</span>
            </div>
            <p class="input-help">Standard amortization period (typically 30 years)</p>
         </div>
         <div class="input-group">
            <label class="input-label">Balloon Payment Term (Years)</label>
             <div class="relative">
                <input v-model.number="sellerFinancingInputs.balloonTerm" type="number" min="1" max="30" class="input-field pr-10" placeholder="Balloon term" />
                <span class="input-symbol-right pr-3">yrs</span>
            </div>
            <p class="input-help">Term for balloon payment (shorter than amortization)</p>
         </div>
     ...
     <div class="space-y-4">
         ...
         <div class="bg-slate-800/70 rounded-lg p-4 border border-slate-700/50 shadow-sm space-y-3">
            ...
            <div class="flex justify-between items-center">
                <p class="text-sm text-slate-300">Monthly P&I Payment:</p>
                <p class="text-lg font-semibold text-blue-300">{{ formatPrice(sellerFinancingCalculated.monthlyPaymentPI) }}/mo</p>
            </div>
         </div>
         <div class="input-group">
            <label class="input-label">Est. Closing Costs (%)</label>
  ...
  <div class="pt-6 mt-6 border-t border-slate-700/50">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
         ...
         <div class="result-box"><div class="result-label">Seller Note Principal</div><div class="result-value">{{ formatPrice(sellerFinancingCalculated.monthlyPrincipal) }}/mo</div></div>
         <div class="result-box"><div class="result-label">Seller Note Interest</div><div class="result-value">{{ formatPrice(sellerFinancingCalculated.monthlyInterest) }}/mo</div></div>
         ...
         <div v-if="sellerFinancingInputs.balloonTerm && sellerFinancingInputs.balloonTerm < sellerFinancingInputs.sellerTerm" class="result-box bg-blue-500/10 border border-blue-500/20">
            <div class="result-label text-blue-300">Balloon Payment</div>
            <div class="result-value text-blue-300">{{ formatPrice(sellerFinancingCalculated.balloonPayment) }}</div>
            <div class="result-detail">Due after {{ sellerFinancingInputs.balloonTerm }} years</div>
         </div>
...
<div v-if="selectedScenario === 'Cash'" class="space-y-6">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
       ...
       <div class="space-y-4">
          ...
          <div class="input-group">
           ...
           <div class="pt-4 mt-2 border-t border-slate-700/50">
              <h5 class="text-sm font-medium text-slate-300 mb-3">Additional Custom Fields</h5>
              <div class="input-group">
                <label class="input-label">Custom Amount 1 ($)</label>
                <div class="relative">
                    <span class="input-symbol-left">$</span>
                    <input v-model.number="cashOfferInputs.customAmount1" type="number" min="0" class="input-field pl-8" placeholder="Enter custom amount"/>
                </div>
              </div>
              <div class="input-group">
                ...
                <div class="relative">
                    <span class="input-symbol-left">$</span>
                    <input v-model.number="cashOfferInputs.customAmount2" type="number" min="0" class="input-field pl-8" placeholder="Enter custom amount"/>
                </div>
              </div>
           </div>
       </div>
      <!-- Cash Inputs Col 2 (Less needed here) -->
       <div class="space-y-4">
           ...
           <div class="bg-slate-800/70 rounded-lg p-4 border border-slate-700/50 shadow-sm space-y-3">
               ...
               <div class="flex justify-between items-center">
                  <p class="text-sm text-slate-300">Additional Costs:</p>
                  <p class="text-lg font-semibold text-blue-300">{{ formatPrice(cashOfferCalculated.additionalCosts) }}</p>
               </div>
           ...
           <div class="input-group mt-4">
              ...
              <textarea v-model="cashOfferInputs.notes" class="input-field p-3 h-24" placeholder="Add any additional notes or considerations for this cash offer"></textarea>
           </div>
       </div>
      <!-- Cash Common Inputs -->
      <div class="md:col-span-2 pt-6 mt-2 border-t border-slate-700/50">
   ...
   <div class="pt-6 mt-6 border-t border-slate-700/50">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
           <div class="result-box"><div class="result-label">Total Cash Needed</div><div class="result-value">{{ formatPrice(cashOfferCalculated.totalCashNeeded) }}</div><div class="result-detail">Offer Price + All Costs</div></div>
...
const calculateMortgagePayment = (principal, annualRatePercent, termYears) => {
  if (!principal || principal <= 0 || !termYears || termYears <= 0) return { payment: 0, principal: 0, interest: 0 };
...
const calculateBalloonPayment = (loanAmount, annualRatePercent, fullTermYears, balloonTermYears) => {
  if (!loanAmount || loanAmount <= 0 || !fullTermYears || fullTermYears <= 0 || !balloonTermYears) return 0;
  ...
  const monthlyPayment = calculateMortgagePayment(loanAmount, annualRatePercent, fullTermYears).payment;
...
const autoFillMortgageData = () => {
  ...
  if (props.property && props.property.mortgage) {
    ...
    if (props.property.mortgage.monthlyPayment) {
      subtoInputs.existingLoanPITI = Number(props.property.mortgage.monthlyPayment);
    }
  } else {
