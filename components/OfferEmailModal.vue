<template>
  ...
  // Get calculated values from the store based on the selected scenario
  const selectedScenario = computed(() => calculatorStore.selectedScenario);
  
  // Values from Subject-To scenario
  const subtoCalculated = computed(() => {
    if (selectedScenario.value === 'Subto') {
      return calculatorStore.subtoCalculated;
    }
    return null;
  });
  
  // Values from Seller Financing scenario
  const sellerFinancingCalculated = computed(() => {
    if (selectedScenario.value === 'SellerFinancing') {
      return calculatorStore.sellerFinancingCalculated;
    }
    return null;
  });
  
  // Values from Cash Offer scenario
  const cashOfferCalculated = computed(() => {
    if (selectedScenario.value === 'Cash') {
      return calculatorStore.cashOfferCalculated;
    }
    return null;
  });
  
  // Computed values from the calculator store
  const calculatedPurchasePrice = computed(() => {
    if (selectedScenario.value === 'Subto') {
      return formatPrice(calculatorStore.subtoInputs.purchasePrice);
    } else if (selectedScenario.value === 'SellerFinancing') {
      return formatPrice(calculatorStore.sellerFinancingInputs.purchasePrice);
    } else if (selectedScenario.value === 'Cash') {
      return formatPrice(calculatorStore.cashOfferInputs.offerPrice);
    }
    return props.formattedPurchaseOrListingPrice;
  });
  
  const calculatedOpenLoanBalance = computed(() => {
    if (selectedScenario.value === 'Subto' && calculatorStore.subtoInputs.existingLoanBalance) {
      return formatPrice(calculatorStore.subtoInputs.existingLoanBalance);
    }
    return props.formattedOpenLoanBalance;
  });
  
  const calculatedCashToSeller = computed(() => {
    if (selectedScenario.value === 'Subto') {
      return formatPrice(calculatorStore.subtoInputs.cashToSeller);
    } else if (selectedScenario.value === 'SellerFinancing') {
      return formatPrice(sellerFinancingCalculated.value.downPaymentAmount);
    } else if (selectedScenario.value === 'Cash') {
      return formatPrice(calculatorStore.cashOfferInputs.offerPrice);
    }
    return props.estimatedValue ? formatPrice(props.estimatedValue * 0.2) : "N/A";
  });
  
  const calculatedSellerCarrybackAmount = computed(() => {
    if (selectedScenario.value === 'Subto') {
      return formatPrice(subtoCalculated.value.sellerCarryAmount);
    } else if (selectedScenario.value === 'SellerFinancing') {
      return formatPrice(sellerFinancingCalculated.value.sellerNoteAmount);
    }
    return "N/A";
  });
  ...
  const calculatedSellerCarrybackInterestRate = computed(() => {
    if (selectedScenario.value === 'Subto') {
      return `${calculatorStore.subtoInputs.sellerCarryRate}%`;
    } else if (selectedScenario.value === 'SellerFinancing') {
      return `${calculatorStore.sellerFinancingInputs.sellerRate}%`;
    }
    return "N/A";
  });
  ...
  const calculatedSellerCarrybackMonthlyPayments = computed(() => {
    if (selectedScenario.value === 'Subto') {
      return formatPrice(subtoCalculated.value.sellerCarryMonthlyPayment);
    } else if (selectedScenario.value === 'SellerFinancing') {
      return formatPrice(sellerFinancingCalculated.value.monthlyPaymentPI);
    }
    return "N/A";
  });
  ...
  const calculatedAgentCommission = computed(() => {
    if (selectedScenario.value === 'Subto') {
      return `${calculatorStore.subtoInputs.commissionPercent}%`;
    } else if (selectedScenario.value === 'SellerFinancing') {
      return `${calculatorStore.sellerFinancingInputs.commissionPercent}%`;
    } else if (selectedScenario.value === 'Cash') {
      return `${calculatorStore.cashOfferInputs.commissionPercent}%`;
    }
    return "N/A";
  });
  ...
  watch(() => props.showModal, (isVisible) => {
    if (isVisible) {
      // Generate the templates with current calculator values
  ...
  // Update the templates when calculator store values or name changes
  watch([
    () => calculatorStore.selectedScenario,
    () => calculatorStore.subtoCalculated,
    () => calculatorStore.sellerFinancingCalculated,
    () => calculatorStore.cashOfferCalculated,
    () => yourName.value
  ], () => {
    if (props.showModal) {
      napkinSignedText.value = generateNapkinSignedTemplate();
