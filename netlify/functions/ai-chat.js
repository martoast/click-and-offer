...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    try {
      ...
      // Run the assistant using GPT-3.5 Turbo for speed
      const run = await openai.beta.threads.runs.create(thread.id, {
        assistant_id: process.env.OPENAI_ASSISTANT_ID || "asst_CaIzxIdsJrvtLjicLpKhZny0",
        model: "gpt-3.5-turbo" // Override with GPT-3.5 Turbo for faster responses
      });
