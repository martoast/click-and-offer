<template>
  <div class="min-h-screen bg-black text-white">
    ...
    <!-- Property Content - Changed condition to check for realEstateData instead of zillowData -->
    <div
      v-else-if="realEstateData"
      class="container mx-auto px-4 pb-16 space-y-8"
    >
...
const zillowData = ref({}); // Initialize with empty object instead of null
...
const fetchPropertyData = async () => {
  ...
  zillowData.value = {}; // Initialize with empty object instead of null
  ...
  try {
    ...
    if (realEstateData.value?.propertyInfo?.address?.label) {
      ...
      // No valid address, skip Zillow API and mark loading as complete
      loading.value = false;
    }
  } catch (err) {
    console.error(
...
const fetchZillowData = async (formattedAddress) => {
  ...
  try {
    ...
    if (!searchData.zpid) {
      console.log("No zpid found in Zillow API response");
      loadingState.zillow = false;
      loading.value = false;
      return; // Exit but keep zillowData as empty object
    }

    console.log(`Found Zillow property with zpid: ${searchData.zpid}`);

    // Now fetch property details using zpid
    await fetchZillowPropertyDetails(searchData.zpid);
  } catch (err) {
...
const fetchZillowPropertyDetails = async (zpid) => {
  ...
  try {
    ...
    // Keep zillowData as an empty object rather than setting to null
    loadingState.zillow = false;
